...
function getStyleDir(compName) {
  ...
  if (!styleDir)
    styleDir = _chunkUX65OJTHcjs.kebabCase.call(void 0, compName);
  return styleDir;
}
...
function resolveComponent(name, options) {
  ...
  const partialName = _chunkUX65OJTHcjs.kebabCase.call(void 0, name.slice(2));
...
function ElementPlusResolver(options = {}) {
  ...
  async function resolveOptions() {
    ...
    optionsResolved = _chunkWJIU4Q36cjs.__spreadValues.call(void 0, {
      ssr: false,
      version: await _chunkUX65OJTHcjs.getPkgVersion.call(void 0, "element-plus", "2.2.2"),
      importStyle: "css",
      directives: true,
      exclude: void 0,
  ...
  return {
    ...
    resolve: (name) => {
      ...
      if (/^El[A-Z]/.test(name)) {
        ...
        const partialName = _chunkUX65OJTHcjs.kebabCase.call(void 0, compName);
  ...
  return {
    ...
    resolve: (name) => {
      ...
      if (!dirname) {
        ...
        dirname = _chunkUX65OJTHcjs.kebabCase.call(void 0, name).split("-")[nameIndex];
      }
...
var moduleType = _chunkUX65OJTHcjs.isSSR ? "lib" : "es";
function getSideEffects4(dirName, options) {
  const { importStyle = true } = options;
  if (!importStyle || _chunkUX65OJTHcjs.isSSR)
    return;
  if (importStyle === "less")
    ...
    resolve: (name) => {
      if (name.startsWith("Van")) {
        ...
        return {
          ...
          sideEffects: getSideEffects4(_chunkUX65OJTHcjs.kebabCase.call(void 0, partialName), options)
        };
      }
    }
  };
...
function getResolved(name, options) {
  ...
  if (importStyle || importCss) {
    if (importStyle === "less" || importLess)
      sideEffects.push(`${path}/es/${_chunkUX65OJTHcjs.kebabCase.call(void 0, name)}/style/less`);
    else
      sideEffects.push(`${path}/es/${_chunkUX65OJTHcjs.kebabCase.call(void 0, name)}/style/index`);
  }
  return {
    from: path,
...
var formatters = {
  "kebab-case": _chunkUX65OJTHcjs.kebabCase,
  "camelCase": _chunkUX65OJTHcjs.camelCase,
  "PascalCase": _chunkUX65OJTHcjs.pascalCase
};
var peerPaths = /* @__PURE__ */ new Map();
function assertPeerPath(peerPath) {
  if (!peerPaths.has(peerPath)) {
    try {
      _chunkUX65OJTHcjs.resolveImportPath.call(void 0, peerPath);
...
function getCompDir(compName) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (compName.match(matcher.pattern)) {
      compPath = `${matcher.compDir}/${_chunkUX65OJTHcjs.kebabCase.call(void 0, compName)}.vue`;
      break;
    }
  }
  if (!compPath)
    compPath = _chunkUX65OJTHcjs.kebabCase.call(void 0, compName);
  return compPath;
}
...
function componentsResolver(name, { ssr }) {
  if (!name.match(/^D[A-Z]/))
    return;
  const resolveId = _chunkUX65OJTHcjs.kebabCase.call(void 0, name = name.slice(1));
  return {
    name,
...
function directivesResolver(name, { ssr }) {
  const resolveId = _chunkUX65OJTHcjs.kebabCase.call(void 0, name);
  return {
    name: `${name}Directive`,
    sideEffects: getSideEffects7(resolveId, "style.css"),
...
function getComponentStyleDir(importName, importStyle) {
  if (["ConfigProvider", "Icon"].includes(importName))
    return void 0;
  let componentDir = _chunkUX65OJTHcjs.kebabCase.call(void 0, importName);
  ...
  return {
    ...
    resolve: (name) => {
      ...
      if (canResolveIcons(options.resolveIcons)) {
        const iconPrefix = _chunkUX65OJTHcjs.pascalCase.call(void 0, getResolveIconPrefix(options.resolveIcons));
