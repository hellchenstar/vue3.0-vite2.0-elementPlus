...
function useGlobalComponentSettings(block, sizeFallback) {
  ...
  const ns = index$1.useNamespace(
    block,
    vue.computed(() => {
      var _a;
      return ((_a = config.value) == null ? void 0 : _a.namespace) || index$1.defaultNamespace;
    })
  );
  const locale = index.useLocale(vue.computed(() => {
    var _a;
...
const provideGlobalConfig = (config, app, global = false) => {
  ...
  provideFn(
    index.localeContextKey,
    vue.computed(() => context.value.locale)
  );
  provideFn(
    index$1.namespaceContextKey,
